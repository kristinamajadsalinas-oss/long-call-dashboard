            ...
            if not df.empty:
                ...
                if table_data:
                    for idx, row_data in enumerate(table_data):
                        ...
                        with cols[5]:
                            # Arrow button to expand details
                        ...
                        # Show expanded details if clicked
                        if st.session_state.get(f'expanded_{idx}', False):
                            engineer = row_data['Engineer']
                            eng_full_data = df[df['engineer'] == engineer].sort_values('timestamp')

                            # Group by Transaction ID
                            for tid in eng_full_data['transactionId'].unique():
                                tid_data = eng_full_data[eng_full_data['transactionId'] == tid].sort_values('timestamp')

                                with st.container(border=True):
                                    st.markdown(f"#### TID: {tid} - {engineer}")
                                    st.caption(f"{len(tid_data)} update(s)")

                                    for _, update_row in tid_data.iterrows():
                                        t = update_row['timestamp'].strftime('%I:%M %p - %b %d')
                                        utype = update_row.get('updateType', 'initial').upper()

                                        st.markdown(f"**{t}** - {utype}")

                                        if update_row.get('updateType') == 'initial':
                                            st.text(f"Issue: {update_row.get('issue', 'N/A')}")
                                            st.text(f"Reason: {update_row.get('reason', 'N/A')}")
                                        else:
                                            st.text(f"Update: {update_row.get('updateText', 'N/A')}")
...
def engineer_view(df):
    ...
    selected_eng = st.selectbox("Select Engineer", engineers)
