            ...
            if not df.empty:
                ...
                for engineer in df['engineer'].unique():
                    ...
                    if time_since < 1:
                        ...
                        last_update = f"{int(time_since)} minutes ago"
                    else:
                        last_update = f"{int(time_since/60)} hours ago"
...
def main():
    ...
    if st.session_state.is_squad_lead:
        # Squad Lead Interface - Icon Navigation
        nav = st.sidebar.radio(
            "Navigation",
            ["🏠 Home", "👥 Squad", "👤 Engineer", "📅 History"],
            key="nav_select"
        )
