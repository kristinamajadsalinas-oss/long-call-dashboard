...
# Real-Time Updates Table - One row per CALL (by Transaction ID)
def show_realtime_table(df):
    ...
    # Group by Transaction ID (one row per call!)
    for tid in df['transactionId'].unique():
        tid_df = df[df['transactionId'] == tid].sort_values('timestamp')

        # Get engineer from first entry
        engineer = tid_df.iloc[0].get('engineer', 'Unknown')

        # Get latest update for this TID
        latest = tid_df.iloc[-1]
        ...
        # Calculate duration (first to last update)
        if len(tid_df) > 1:
            dur_sec = (tid_df.iloc[-1]['timestamp'] - tid_df.iloc[0]['timestamp']).total_seconds()
            dur_str = f"{int(dur_sec // 60):02d}:{int(dur_sec % 60):02d}"
        else:
            dur_str = "—"

        # Last update text (first line only)
        if latest.get('updateType') == 'initial':
            last_text = latest.get('issue', 'N/A')
        else:
            last_text = latest.get('updateText', latest.get('reason', 'N/A'))
        ...
        # Store full timeline data by TID
        call_data_map[tid] = tid_df

    # Display table with row selection (click row to view timeline)
    ...
    # When row is selected, show full timeline
    if event and event.selection and event.selection.rows:
        selected_idx = event.selection.rows[0]
        selected_tid = table_rows[selected_idx]['TID']
        selected_eng = table_rows[selected_idx]['Engineer']
        tid_df = call_data_map[selected_tid]

        st.markdown("---")
        st.markdown(f"### Full Entry - {selected_eng} - TID: {selected_tid}")

        for _, row in tid_df.iterrows():
            # Full timestamp with date
            timestamp_full = row['timestamp'].strftime('%b %d, %Y - %I:%M %p')

            if row.get('updateType') == 'initial':
                st.markdown(f"### {timestamp_full}")
                st.markdown(f"**Type:** INITIAL UPDATE")
                st.markdown(f"**Transaction ID:** {selected_tid}")
                st.markdown(f"**Issue:**")
                st.text(row.get('issue', 'N/A'))  # Use st.text for plain display
                st.markdown(f"**Reason:**")
                st.text(row.get('reason', 'N/A'))
            else:
                st.markdown(f"### {timestamp_full}")
                st.markdown(f"**Type:** PROGRESS UPDATE")
                st.markdown(f"**Update:**")
                # Show ALL update text clearly (no fading!)
                update_text = row.get('updateText', 'N/A')
                st.text(update_text)  # Plain text, no markdown formatting that could fade
        ...
        with st.expander(f"TID: {tid} • {date} • {len(tid_df)} update(s)", expanded=False):
            for idx, row in tid_df.iterrows():
                ...
                if row.get('updateType') == 'initial':
                    ...
                    # Show current content clearly
                    st.markdown(f"**Issue:**")
                    st.text(row.get('issue', 'N/A'))
                    ...
                    # Show current content clearly (NO FADING!)
                    st.markdown(f"**Update:**")
                    st.text(row.get('updateText', 'N/A'))
    ...
    for tid in eng_df['transactionId'].unique():
        ...
        with st.expander(f"TID: {tid} • {date} • {len(tid_df)} update(s)"):
            for _, row in tid_df.iterrows():
                ...
                if row.get('updateType') == 'initial':
                    st.markdown(f"### {timestamp_full} - INITIAL UPDATE")
                    st.markdown(f"**Transaction ID:** {tid}")
                    st.markdown(f"**Issue:**")
                    st.markdown(f"{row.get('issue', 'N/A')}")
                    st.markdown(f"**Reason:**")
                    st.markdown(f"{row.get('reason', 'N/A')}")
                else:
                    st.markdown(f"### {timestamp_full} - PROGRESS UPDATE")
                    st.markdown(f"**Update:**")
                    # Show full update text (no fading!)
                    update_text = row.get('updateText', 'N/A')
                    if update_text and update_text != 'N/A':
                        st.markdown(f"{update_text}")
                    else:
                        st.markdown("No update text")
