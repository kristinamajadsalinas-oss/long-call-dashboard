...
def home_view(df):
    ...
    # Layout (left panel wider, right panel narrower)
    ...
    with left_col:
        with st.container(border=True):
            ...
            if not df.empty:
                ...
                # Display as clean table using Streamlit dataframe
                if table_data:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Engineer": st.column_config.TextColumn("Engineer", width="medium"),
                            "Current Status": st.column_config.TextColumn("Current Status", width="medium"),
                            "Duration": st.column_config.TextColumn("Duration", width="small"),
                            "Last Update": st.column_config.TextColumn("Last Update", width="large")
                        }
                    )

                    # Add clickable arrows below table
                    st.caption("Select an engineer to view full timeline:")
                    row_cols = st.columns(len(table_data))
                    for idx, row_data in enumerate(table_data):
                        with row_cols[idx]:
                            if st.button(f"→ {row_data['Engineer']}", key=f"view_{idx}", use_container_width=True):
