...
def engineer_view(df, engineer_name):
    ...
    # All Updates Table (for Engineer View)
    st.markdown("---")
    st.subheader("📋 All My Updates")

    if not engineer_df.empty and 'timestamp' in engineer_df.columns:
        display_cols = ['timestamp', 'transactionId', 'squad', 'skillset', 'updateType']
        available_cols = [col for col in display_cols if col in engineer_df.columns]
        display_df = engineer_df[available_cols].copy()
