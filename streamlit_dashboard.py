...
# Real-Time Updates Table (SHARED by squad leads and engineers!)
def show_realtime_table(df):
    ...
    # Clickable engineer selection to view timeline
    st.markdown("**Click an engineer to view full timeline:**")

    selected_eng = st.selectbox("Select Engineer", df['engineer'].unique(), label_visibility="collapsed", key="timeline_select")

    if selected_eng:
        # Get this engineer's latest TID
        eng_data = df[df['engineer'] == selected_eng].sort_values('timestamp', ascending=False)
        tid = eng_data.iloc[0].get('transactionId')
        tid_df = df[df['transactionId'] == tid].sort_values('timestamp')

        with st.expander(f"**Full Timeline - {selected_eng} - TID: {tid}**", expanded=True):
        ...
        with st.expander(f"TID: {tid} • {date} • {len(tid_df)} update(s)", expanded=False):
            for idx, row in tid_df.iterrows():
                ...
                if row.get('updateType') == 'initial':
                    ...
                    # Editable fields
                    new_issue = st.text_area(f"Issue (editable)", row.get('issue', ''), key=f"issue_{idx}", height=80)
                    new_reason = st.text_area(f"Reason (editable)", row.get('reason', ''), key=f"reason_{idx}", height=100)
                    
                    if st.button("💾 Save Changes", key=f"save_{idx}"):
                        # Update Firebase here (future feature)
                        st.success("Changes saved!")
                else:
                    st.markdown(f"**{t}** - PROGRESS UPDATE")
                    new_update = st.text_area(f"Update (editable)", row.get('updateText', ''), key=f"update_{idx}", height=80)
...
def engineer_view(df):
    ...
    for tid in eng_df['transactionId'].unique():
        ...
        with st.expander(f"TID: {tid} • {date}"):
            for _, row in tid_df.iterrows():
                t = row['timestamp'].strftime('%I:%M %p')
                st.markdown(f"**{t}** - {row.get('updateType', 'initial').upper()}")
                st.markdown("---")
