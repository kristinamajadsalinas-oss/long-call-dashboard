...
def show_realtime_table(df):
    ...
    st.dataframe(table_df, use_container_width=True, hide_index=True, height=400, on_select="rerun", selection_mode="single-row")

    # When a row is selected, show timeline below
    if st.session_state.get('selection'):
        ...
        if selected_rows:
            ...
            st.markdown(f"**Timeline for {selected_eng}**")
