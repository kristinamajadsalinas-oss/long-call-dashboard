        ...
        with st.container(border=True):
            ...
            if not df.empty:
                ...
                for engineer in df['engineer'].unique():
                    ...
                    # Last update TEXT (not time) - FIRST LINE ONLY
                    ...
                    # Take only first line and truncate
                    if isinstance(last_update_text, str):
                        last_update_text = last_update_text.split('\n')[0].split('>')[0].strip()
                        if len(last_update_text) > 50:
                            last_update_text = last_update_text[:50] + "..."
                    else:
                ...
                # Display as clean HTML table (no Streamlit columns causing alignment issues)
                if table_data:
                    # Build HTML table
                    table_html = """
                    <table style='width:100%;border-collapse:collapse;'>
                        <thead>
                            <tr style='background-color:#F8F9FA;border-bottom:2px solid #DEE2E6;'>
                                <th style='padding:12px;text-align:left;font-weight:600;color:#495057;width:5%;'>Status</th>
                                <th style='padding:12px;text-align:left;font-weight:600;color:#495057;width:20%;'>Engineer</th>
                                <th style='padding:12px;text-align:left;font-weight:600;color:#495057;width:15%;'>Current Status</th>
                                <th style='padding:12px;text-align:left;font-weight:600;color:#495057;width:12%;'>Duration</th>
                                <th style='padding:12px;text-align:left;font-weight:600;color:#495057;width:43%;'>Last Update</th>
                                <th style='padding:12px;text-align:center;font-weight:600;color:#495057;width:5%;'>View</th>
                            </tr>
                        </thead>
                        <tbody>
                    """
                    ...
                    for idx, row_data in enumerate(table_data):
                        ...
                        <tr style='border-bottom:1px solid #E9ECEF;' onmouseover="this.style.backgroundColor='#F8F9FA'" onmouseout="this.style.backgroundColor='white'">
                            <td style='padding:12px;font-size:1.2rem;'>{row_data['Status']}</td>
                            <td style='padding:12px;font-weight:600;'>{row_data['Engineer']}</td>
                            <td style='padding:12px;'>{row_data['Current Status']}</td>
                            <td style='padding:12px;font-family:monospace;font-weight:600;'>{row_data['Duration']}</td>
                            <td style='padding:12px;'>{row_data['Last Update']}</td>
                            <td style='padding:12px;text-align:center;'><span style='cursor:pointer;font-size:1.2rem;color:#D71921;'>→</span></td>
                        </tr>
                        """
                    ...
                    # Note: Arrow is visual only in HTML table, will add clickable version below
                    st.caption("Click arrow to view engineer's full timeline")
            else:
    ...
    for tid in filtered['transactionId'].unique():
        ...
        with st.expander(f"TID: {tid} • {first} • {update_count} update(s)"):
            for _, row in tid_data.iterrows():
                ...
                utype = str(row.get('updateType', 'initial')).upper()
