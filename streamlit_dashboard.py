...
<style>
    ...
    html, body, .main, [data-testid="stApp"], [data-testid="stAppViewContainer"],
    [data-testid="stDecoration"], [data-testid="stToolbar"], section {
...
# Squad Lead List (Authorized Users)
SQUAD_LEADS = ["chris", "judith", "josh", "bea"]

# Login with username and password
def check_login():
    """Username-based login with role detection"""
    ...
    if not st.session_state.authenticated:
        ...
        st.markdown("Please enter your credentials to access")

        username = st.text_input("Username", placeholder="e.g., kristinamajasa or judith")
        password = st.text_input("Password", type="password")

        if st.button("Login", use_container_width=True):
            if password and hashlib.sha256(password.encode()).hexdigest() == CORRECT_PASSWORD_HASH:
                if username.strip():
                    # Determine role
...
# ENGINEER'S VIEW
# ============================================
def engineer_view(engineer_siebel):
    """Engineer's personal dashboard - shows only their own data"""

    st.title("Long Call Updates")
    st.markdown(f"##### {engineer_siebel}'s Personal Dashboard")
...
# ============================================
def main():
    # Check login - returns False if not authenticated
    if not check_login():
        return
    ...
    # Auto-route to correct view based on role (NO TOGGLE!)
    if role == "squad_lead":
        # Squad leads see team dashboard
        squad_lead_view()
    else:
        # Engineers see personal dashboard with their username
        engineer_view(username)
